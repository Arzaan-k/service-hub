...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ORBCOMM ALL devices endpoint (unfiltered) - returns ALL devices, not just those with telemetry
  app.get('/api/orbcomm/all-devices', async (req, res) => {
    try {
      console.log('📱 ORBCOMM ALL devices endpoint called');
      const { getOrbcommClient } = await import('./services/orbcomm-real');
      const orbcommClient = getOrbcommClient();

      console.log('🔍 ORBCOMM client connected:', orbcommClient.isConnected);
      console.log('📱 Calling getAllDevicesUnfiltered...');

      const devices = await orbcommClient.getAllDevicesUnfiltered();
      console.log('📱 Retrieved ALL ORBCOMM devices:', devices.length);
      console.log('📱 Device IDs:', devices.map(d => d.deviceId));

      res.json(devices);
    } catch (error) {
      console.error('❌ ORBCOMM ALL devices error:', error);
      console.error('❌ Error stack:', error instanceof Error ? error.stack : 'No stack');
      res.status(500).json({
        error: 'Failed to fetch ALL ORBCOMM devices',
        details: error instanceof Error ? error.message : 'Unknown error',
        stack: error instanceof Error ? error.stack : undefined
      });
    }
  ...
  // ORBCOMM raw events endpoint - get all events received (not filtered by container matching)
  app.get('/api/orbcomm/raw-events', async (req, res) => {
    try {
      console.log('📡 ORBCOMM raw events endpoint called');
      const { getOrbcommClient } = await import('./services/orbcomm-real');
      const orbcommClient = getOrbcommClient();

      console.log('🔍 ORBCOMM client connected:', orbcommClient.isConnected);
      console.log('📡 Calling getAllRawEvents...');

      const rawEvents = await orbcommClient.getAllRawEvents();
      console.log(`📡 Retrieved ${rawEvents.length} raw ORBCOMM events`);

      res.json({
        success: true,
        count: rawEvents.length,
        events: rawEvents,
        message: 'All raw ORBCOMM events received (unfiltered by container matching)'
      });
    } catch (error) {
      console.error('❌ ORBCOMM raw events error:', error);
      console.error('❌ Error stack:', error instanceof Error ? error.stack : 'No stack');
      res.status(500).json({
        error: 'Failed to fetch ORBCOMM raw events',
        details: error instanceof Error ? error.message : 'Unknown error',
        stack: error instanceof Error ? error.stack : undefined
      });
    }
  ...
  // Live ORBCOMM data without authentication - returns all available data
  app.get('/api/orbcomm/live-data-open', async (req, res) => {
    try {
      console.log('🚀 Fetching live ORBCOMM data without authentication');
      ...
      // Get all ORBCOMM devices (unfiltered)
      const orbcommDevices = await orbcommClient.getAllDevicesUnfiltered();
      console.log(`📡 Retrieved ${orbcommDevices.length} ORBCOMM devices`);

      // Get all containers from database (optional for matching)
      let allContainers = [];
      try {
        const { storage } = await import('./storage');
        allContainers = await storage.getAllContainers();
        console.log(`📦 Retrieved ${allContainers.length} containers from database`);
      } catch (error) {
        console.log('⚠️ Could not fetch containers, proceeding without matching');
      }

      const reeferUnits = [];
      const deviceStatus = [];

      // Process each ORBCOMM device
      for (const orbcommDevice of orbcommDevices) {
        ...
        // Find matching container by Reefer ID (AssetID) - optional
        ...
        if (!matchingContainer) {
          console.log(`⚠️ No matching container found for Reefer ID: ${deviceData.lastAssetId || orbcommDevice.deviceId}, but proceeding anyway`);
        } else {
        ...
        const doorStatus = deviceData.doorStatus;

        // Extract alarms
        const alarms = deviceData.errorCodes || [];

        // Create reefer unit data
        const reeferUnit = {
          deviceId: orbcommDevice.deviceId,
          containerId: matchingContainer?.containerCode || orbcommDevice.deviceId,
          temperature: temperature,
          powerStatus: powerStatus,
          batteryLevel: batteryLevel,
          doorStatus: doorStatus,
          alarms: alarms,
          lastUpdate: deviceData.lastUpdate || orbcommDevice.lastSeen,
          oem: deviceData.oem || 'ORBCOMM',
          location: deviceData.location || orbcommDevice.location,
          rawData: deviceData.rawData || deviceData
        };
        ...
        const deviceState = {
          ...
          lastCommunication: deviceData.lastUpdate || orbcommDevice.lastSeen,
          signalStrength: deviceData.signalStrength || 'N/A',
          firmwareVersion: deviceData.firmwareVersion || 'N/A',
          reportingInterval: deviceData.reportingInterval || '15 min'
        };

        reeferUnits.push(reeferUnit);
        deviceStatus.push(deviceState);
      }
      ...
      res.json({
        success: true,
        dataSource: 'REAL_ORBCOMM_LIVE_DATA_OPEN',
        ...
        message: 'Live ORBCOMM data fetched without authentication. Container matching is optional.'
      });

    } catch (error) {
      console.error('❌ ORBCOMM live data (open) error:', error);
      res.status(500).json({
