...
class OrbcommAPIClient {
  ...
  // Get all raw events received from ORBCOMM (not filtered by container matching)
  async getAllRawEvents(): Promise<any[]> {
    console.log(`📡 getAllRawEvents called - CDH connected: ${this.isConnected}`);
    
    if (!this.isConnected) {
      console.log('⚠️ CDH not connected, returning empty array');
      return [];
    }
    
    // Return all events we've received, not just those with container matches
    const allEvents = [];
    for (const device of this.devices) {
      if ((device as any).rawEvent) {
        allEvents.push({
          deviceId: device.deviceId,
          event: (device as any).rawEvent,
          processed: true
        });
      }
    }
    
    console.log(`📡 Returning ${allEvents.length} raw ORBCOMM events`);
    return allEvents;
  }
  ...
  // Get ALL devices (read-only) - returns all ORBCOMM devices without filtering
  async getAllDevicesUnfiltered(): Promise<OrbcommDevice[]> {
    console.log(`📱 getAllDevicesUnfiltered called - CDH connected: ${this.isConnected}, cached devices: ${this.devices.length}`);
    ...
    // Return ALL devices without filtering - even those without current telemetry
    console.log(`📱 Returning ${this.devices.length} total ORBCOMM devices (unfiltered)`);
    return this.devices;
  }
